...
---
--@param screen
--@param layer screen name
--@param key_template control name
--@param target_var the storyboard variable to update as the keyboard receives input.
--@param default_language the default language to which the UI will be initialized.
--@param label_font the default font for labels
--@param target_ans variable that should update with answer when enter key is pressed (hopefully)
function VKI.Init(screen, layer, key_template, target_var, default_language, label_font, language_list_table, target_ans, target_prev)
  local vki = {}
  setmetatable(vki, VKI)
  ...
  vki.VKI_answer = target_ans
  vki.VKI_previous = target_prev
  
  vki.cached_controls = {}

  --CRANK TF: Set the default keyboard
  --vki.VKI_kts = this.VKI_kt = param[0];
  vki.VKI_kt = default_language
  vki.VKI_kts = vki.VKI_kt
...
function VKI:SetEmulation(on)
  self.VKI_emulate = on
end

--- Get the keyboard types associated with this virtual keyboard
---@return An array of keyboard types supported by this class
function VKI:KeyboardTypeList()
  local types = {}
  for k,v in pairs(self.VKI_layout) do
...
---@param type The type of keyboard to get the name of
---@return A string containing the keyboard name
...
---@return A string containing the current keyboard name
...
---@param type The new type of the keyboard to select
function VKI:ChangeKeyboardType(type)
    self.VKI_kt = type;
    self.VKI_kts = self.VKI_kt
    self:VKI_buildKeys()
end
...
-- @return The name of a control to use for this key (may not be visible)
...
--- @param controlName The name of the control we're setting
--- @param clr The new color variable we are setting
--- @param dataTable An optional table to set instead of setting the variable directly 
...
function VKI:VKI_evaluate() 
  local value = gre.get_value(self.VKI_target)
  io.open("C:\\calc\\code\\raceconditionscalculator\\code\\parse.txt","w"):close()
  file2 = io.open("C:\\calc\\code\\raceconditionscalculator\\code\\parse.txt", 'w')
  file2:write(value)
  file2:close()
--  strProgram = '"C:\\calc\\code\\raceconditionscalculator\\code\\calc\\eval.exe"'
--  strCmd = 'start "" '..strProgram
  os.execute("start eval.exe")
-- --io.close(file2)
  file = io.open("C:\\calc\\code\\raceconditionscalculator\\code\\trans.txt",'r')
  gre.set_value(self.VKI_answer, file:read())
  gre.set_value(self.VKI_target, "")
  gre.set_value(self.VKI_previous, value)
  file:close()
  io.open("C:\\calc\\code\\raceconditionscalculator\\code\\trans.txt","w"):close()
  
--  gre.set_value(self.VKI_answer, value)
--  gre.set_value(self.VKI_target, "")
--  gre.set_value(self.VKI_previous, value)
end
...
function RowLayout(x, width, pad, controlList, center)
